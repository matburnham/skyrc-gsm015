...
def find_device_endpoints():
  dev = skyrc_usb.core.find(idVendor=0x28E9, idProduct=0x028A)
  if dev is None:
  ...
  ep_out = skyrc_usb.util.find_descriptor(
      intf,
      # match the first OUT endpoint
      custom_match = \
      lambda e: \
          skyrc_usb.util.endpoint_direction(e.bEndpointAddress) == \
          skyrc_usb.util.ENDPOINT_OUT)
  assert ep_out is not None

  ep_in = skyrc_usb.util.find_descriptor(
      intf,
      # match the first IN endpoint
      custom_match = \
      lambda e: \
          skyrc_usb.util.endpoint_direction(e.bEndpointAddress) == \
          skyrc_usb.util.ENDPOINT_IN)
...
def read_data_from_device(dev, ep_in):
  ...
  except skyrc_usb.core.USBError as e:
